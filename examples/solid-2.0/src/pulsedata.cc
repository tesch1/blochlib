...
// THe pulse matrixs
hmatrixs PulseData::Pulse(SolidSys &A)
{
	matrixs tm=A.F0();
...
ostream &operator<<(ostream &oo,const PulseData &out)
{
	out.print(oo, false);
    return oo;
}
...
{
	BLEXCEPTION(std::string(" Bad 'pulse' usage for \"")+mySec+"\""+
	+"\n should be \"spin:pulse(time)\" \"spin:pulse(time, phase)\", "
	+"\n \"spin:pulse(time, phase, amp)\" or \"spin:pulse(pulse(time, phase, amp,offset)\"")
}

//the dealy error function
void PulseData::DelayErr(std::string mySec,const char * file, int line)
{
	BLEXCEPTION(std::string(" Bad 'delay' usage for \"")+mySec+"\""+
	"\n should be \"spin:delay(time)\" " )
}
