 ...
 */
 ...
 /*
 	...
 	stencilbasic.h-> the basic 'application' of stencils
 		This can get a bit tricky...There are 2 basic
 		types of 'stencils' that i need to impliment
 		the first type is meant for containers with 'wellformed'
 		beingings, ends, and steps sizes (i.e. Vector, Matrix)

 		the other uses the 'stencilprep' class where boundries,
 		edges, neighbors, etc are NOT formed simply by the
 		edges of the container itself
...
#include "stencils/stencilmap.h"
#include "stencils/stencilextent.h"

BEGIN_BL_NAMESPACE


//#include "stencils/stencilfactor.h"

// The basic Stencil container...
// for use with Vector, coord<> and matrix...

template<class Container_t, class NumType_t, int Dim>
class StencilBasic : public StencilExtent<Dim>
{
	public:
	//	typedef Container_t Container_t;
		typedef NumType_t numtype;
		static const int datalength=Dim;
	...
	public Vector<NumType_t>::iterator
{
	private:
		...
		//need works::
		NumType_t *data_;	//need to do this becuase of some stupidity with 'const' functions
							//inside the 'VectorIterator'...BOO! on this cludge...
...
//second derivative to 1st order nomalized
VectorDerivsBEG(DerivativeB_2_1n_cl, A, -2,0){
	return  A.shift(0)+A.shift(-2)-2.0*A.shift(-1);
}
...
VectorDerivsBEG(Derivative_2_2, A, -1,1){
	return  A.shift(-1)-A.shift(0)+A.shift(1);
}
...
VectorDerivsBEG(Derivative_2_4, A, -2,2){
	return  16.0*A.shift(-1)-A.shift(-2)-30.0*A.shift(0)+16.0*A.shift(1)-A.shift(2);
}

//second derivatative to 4th order normalized
VectorDerivsBEG(Derivative_2_4n, A, -2,2){
	return  stencil_r12*(16.0*A.shift(-1)-A.shift(-2)-30.0*A.shift(0)+16.0*A.shift(1)-A.shift(2));
}
...
VectorDerivsBEG(Derivative_3_4, A, -2,2){
	return  13*A.shift(-1)-8.0*A.shift(-2)-13.0*A.shift(1)+8.0*A.shift(2);
}

//third derivatative to 4th order normalized
VectorDerivsBEG(Derivative_3_4n, A, -2,2){
	return  stencil_r8*(13*A.shift(-1)-8.0*A.shift(-2)-13.0*A.shift(1)+8.0*A.shift(2));
}
...
VectorDerivsBEG(Derivative_4_4, A, -2,2){
	return  12.0*A.shift(-2)-39.0*A.shift(-1)+56.0*A.shift(0)-39.0*A.shift(1)+12.0*A.shift(2);
}

//forth derivatative to 4th order normalized
VectorDerivsBEG(Derivative_4_4n, A, -2,2){
	return  stencil_r6*(12.0*A.shift(-2)-39.0*A.shift(-1)+56.0*A.shift(0)-39.0*A.shift(1)+12.0*A.shift(2));
}
