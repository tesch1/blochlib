 ...
 */
 ...
 /*
 	...
 	boundary_edgeblend.h-> takes a corner of an object
...
#ifndef _boundary_cornerblend_h_
#define _boundary_cornerblend_h_ 1
	...
	public BoundaryGeneral
{
	...
	public:
		...
		CornerBlendBC(const numtype &in):
			BoundaryGeneral(),
			const_(in)
		{}

		template<class Shape_t, int Dim>
		CornerBlendBC(const numtype &in,StencilPrep<Shape_t, Dim> &sph,
			int inopts=BCparams::AllFaces | BCparams::AllFaceNorms ):
			BoundaryGeneral(sph,(!(inopts&BCparams::CornersOnly)?(inopts | BCparams::CornersOnly):inopts)),
			const_(in)
		{}

		template<class Shape_t, int Dim, class Shapeexpr>
		CornerBlendBC(const numtype &in,StencilPrep<Shape_t, Dim> &sph, const ShapeExpr<Shapeexpr> &se,
			int inopts=BCparams::AllFaces | BCparams::AllFaceNorms):
			BoundaryGeneral(sph,se,(!(inopts&BCparams::CornersOnly)?(inopts | BCparams::CornersOnly):inopts) ),
			const_(in)
		{}
		...
		{
			//the 'corner selection' should have already been made in the 'BoundaryGeneral' class
			int i=0;
			CornerPos<Dim> myc=test(myit()).edge.CornerIdx();
			if(myc.idx != -1)
			{
				double we=1./3.;
				alter[myit()]=0.0;
				for(i=0;i<Dim;++i)
				{ alter[myit()]+=we*alter[test(myit()).near(myc.dir[i], myc.idx[i])];	}
				return;
			}
		}
/*
