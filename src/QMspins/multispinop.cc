...
#ifndef _multispinop_cc_
#define _multispinop_cc_ 1
...
mxT mps_expand(spin_sysPars &A, mxT &inop, int where)
{
	...
	for(i=0;i<where;i++)
	{
		rimatrix tm=Ie(A.HS(i));
		loo=cross(tm, out);
		out=loo;
	}
	if((where+1)==A.size()) return out;
	for(i=where+1;i<A.size();i++)
	{
		rimatrix tm=Ie(A.HS(i));
		loo=cross(out,tm);
		out=loo;
...
{
	rmatrix tm=Imi(A.HS(spin));
	return mps_expand(A, tm, spin);
}

rimatrix Ie(spin_sysPars &A, int spin){	return mps_Ie(A, spin);	}

//Ix matrix (symmetric/real) matrix
smatrix Ix(spin_sysPars &A, int spin){	return mps_Ix(A, spin);	}

//Iy matrix (hermitian/complex) matrix
hmatrix Iy(spin_sysPars &A, int spin){	return mps_Iy(A, spin);	}

//Iz matrix (real digonal) matrix
rdmatrix Iz(spin_sysPars &A, int spin){	return mps_Iz(A, spin);	}

//Ip matrix (real full) matrix
rmatrix Ip(spin_sysPars &A, int spin){	return mps_Ip(A, spin);	}

//Im matrix (real full) matrix
rmatrix Imi(spin_sysPars &A, int spin){	return mps_Imi(A, spin);	}
