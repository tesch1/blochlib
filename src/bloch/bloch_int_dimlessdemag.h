...
#ifndef _bloch_interatction_Dim_Less_demag_field_h_
#define _bloch_interatction_Dim_Less_demag_field_h_ 1
...
class DimLessDemagField {
	...
	public:
		...
		DimLessDemagField(GridEng_t &gr, double fact=0.0):
			gridm(&gr), i_on(true), func(0), fact_(fact)
		{
			Bd_.resize(gr.size());
		}

		DimLessDemagField(GridEng_t &gr, ScaleFunc_t &sc, double fact=0.0):
			gridm(&gr), i_on(true), func(&sc), fact_(fact)
		{
			Bd_.resize(gr.size());
		}

		DimLessDemagField(const DimLessDemagField &cp):
			gridm(cp.gridm), Bd_(cp.Bd_), i_on(cp.i_on), func(cp.func), fact_(cp.fact_)
		{}

		inline DimLessDemagField &operator=(const DimLessDemagField &cp);
		...
		void SetFactor(double in){	fact_=in;	}
...
//assignments....
template<class GridEng_t, class ScaleFunc_t>
inline DimLessDemagField<GridEng_t,ScaleFunc_t> &
  DimLessDemagField<GridEng_t,ScaleFunc_t>::operator=(const DimLessDemagField<GridEng_t,ScaleFunc_t> &cp)
{
...
template<class GridEng_t, class ScaleFunc_t>
template<class PList>
inline void DimLessDemagField<GridEng_t,ScaleFunc_t>::preCalculate(
	...
	coord<> &totM) 
{
	if(i_on && fact_)
	{
		if(!gridm)
		{
			std::cerr<<std::endl<<"Warning: DimLessDemagField::preCalculate(...)"<<std::endl;
			...
			while(GiterT)
			{
				if(j!=i)
				{
					...
					*/
					...
					if(diffLen)
					{
						//we divide out by the total magnetization to
						// normalize our interaction with repsect to the
						// magnetization
						...
						//cout<<GiterT.CellVolume()<<" PRE: "<<Bd_[i]<<" diflem: " <<diffLen<<" costh: "<<cosTH<<" fact: "<<fact_<<endl;
...
template<class GridEng_t, class ScaleFunc_t>
template<class ParamIter>
inline rmatrix DimLessDemagField<GridEng_t,ScaleFunc_t>::jacobian(
	double t,coord<>  &M,
	ParamIter *pars,
	coord<> &totM)
{
...
template<class GridEng_t, class ScaleFunc_t>
template<class ParamIter>
inline void DimLessDemagField<GridEng_t,ScaleFunc_t>::jacobian(
	rmatrix &out,
	double t,coord<>  &M,
	ParamIter *pars,
	coord<> &totM)
...
template<class GridEng_t, class ScaleFunc_t>
template<class ParamIter>
inline coord<> DimLessDemagField<GridEng_t,ScaleFunc_t>::function(
	double t,
	coord<>  &M,
	coord<>  &dMdt,
	ParamIter *pars,
...
template<class GridEng_t, class ScaleFunc_t>
template<class PList>
inline void DimLessDemagField<GridEng_t,ScaleFunc_t>::function(
	double t,
	Vector<coord<> >  &M,
	Vector<coord<> >  &dMdt,
	PList *pars,
