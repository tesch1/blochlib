...
template<class T>
void dummyf(T &op, T &oo, double moo){ 	}

void dummyf(){ 	}

template<class T, int N>
void coorddax(coord<T, N> &a, coord<T, N> &b, T con)
{
	double cp=-con;
	a+=con*b;
	a+=cp*b;
...
#define DAXPY(N) \
	...
	std::string presstr ## N= " precision: double";	\
	coord<double, N> a ## N, b ## N, c ## N;	\
	double con ## N=Rand(), cp ## N=-con ## N;	\
	for(int i=0;i<N;++i){	a ## N(i)=Rand();b ## N(i)=Rand();c ## N(i)=Rand();	}	\
	LOOPER(a ## N += con ## N * b ## N  ;a ## N += cp ## N * b ## N   ;, dummyf();,  4*N*2,  "a+=const*b..."+lenstr ## N+presstr ## N, daxpyF)	\
